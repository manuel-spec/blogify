...
const blogUrl = 'http://10.240.69.102:9000/api/blogs';
...
Future<ApiResponse> getBlogs() async {
	...
	try {
		...
		switch (response.statusCode) {
			case 200:
				apiResponse.data = jsonDecode(response.body)['blogs'].map((b) => Blog.fromJson(b)).toList();
...
Future<ApiResponse> toggleLike(int blogId) async {
	...
	try {
		...
		final response = await http.post(
		Uri.parse("$blogUrl/$blogId/likes"),
			headers: {
				'Accept': 'application/json',
				'Authorization': 'Bearer $token',
			}
